...
`tools/gen-version.ps1` writes `src/BuildInfo.generated.cs` before every compile, called from
both `build.cmd` and `tests/run-tests.cmd`. The file is gitignored — never commit it, never
hand-edit it.

- **patch** = `git rev-list --count HEAD`, so it rises with every commit
- **commit** = short hash, with a trailing `+` when the working tree is dirty, meaning the
  binary does not correspond exactly to that commit
- **built** = timestamp, which is what separates two builds of the same commit
